...
public class TestMergeLifeConfig {
    ...
    public void testBadIntConfig() throws IOException {
        ...
        new MergeLifeConfig(file.toString());
    }
    ...
    @Test
    public void testPaperObjective() {
        MergeLifeConfig config = MergeLifeConfig.paperObjective();
        Assert.assertEquals(100, config.getRows());
        Assert.assertEquals(100, config.getCols());
        Assert.assertEquals(100, config.getPopulationSize());
        Assert.assertEquals(3.5, config.getScoreThreshold(), 0.001);
        Assert.assertEquals(5, ((BasicObjectiveFunction) config.getObjectiveFunction()).getStats().size());
    }
}
